...
build/debootstrap/.stage2: build/debootstrap/.stage1 | build/debootstrap/
	DEBOOTSTRAP_DIR=$(shell pwd)/debootstrap sudo qemu-aarch64 -L $(shell pwd)/$(dir $<) -E LD_LIBRARY_PATH=$(shell pwd)/lib:$(shell pwd)/usr/lib/aarch64-linux-gnu $(shell pwd)/$(dir $<)/usr/sbin/chroot $(shell pwd)/build/debootstrap debootstrap/debootstrap --second-stage
	touch $@
